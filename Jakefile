...
function prepDirs () {
  ...
  jake.rmRf(__dirname + "../htdocs/commands");
  jake.rmRf(__dirname + "../htdocs/controllers");
  jake.rmRf(__dirname + "../htdocs/lib");
  jake.rmRf(__dirname + "../htdocs/models");
  jake.rmRf(__dirname + "../htdocs/views");
  jake.mkdirP(__dirname + "../htdocs/commands");
  jake.mkdirP(__dirname + "../htdocs/controllers");
  jake.mkdirP(__dirname + "../htdocs/lib");
  jake.mkdirP(__dirname + "../htdocs/models");
  jake.mkdirP(__dirname + "../htdocs/views");
...
namespace("build", function() {
  ...
  task("module", [], function() {
    ...
    runCmds(cmds, function() {
      ...
      all(
        partial(copyAndMinifyFile, __dirname + "/src/require.js"),
        partial(copyFile, __dirname + "/src/index.html"),
        partial(buildFiles, __dirname + "/src/client.js")
      ).then(function() {
...
isFile = partial(promise, function (name, p) {
...
readFile = partial(promise, function (file, p) {
...
function move(src, dst, file) {

  file.name = file.name.replace(src, dst);
...
createFolder = partial(promise, function (file, p) {
  ...
  runCmds(["mkdir -p " + folder], bind(p, p.resolve, file));

}


writeFile = partial(promise, function (file, p) {
...
function minifyFile(file) {
  ...
  ast = ujs.parser.parse(file.data);      // parse code and get the initial AST
  ...
  file.data = ujs.uglify.gen_code(ast);   // compressed code here
